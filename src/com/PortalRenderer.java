...
public final class PortalRenderer {
	...
	public final void renderTextures(Renderer g3d, House house) {
		...
		for(int i = 0; i < PortalManager.COUNT; i++) {
			...
			// одиночный портал (пары ещё нет) или игрок вплотную к порталу,
			// когда экранные координаты вершин посчитать нельзя -> заливка цветом
			if(!linked || state == PortalManager.NEAR_CLIPPED) {
				mode[i] = MODE_FLAT;
				continue;
			}
